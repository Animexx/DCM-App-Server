<?php
namespace DCM\V1\Rest\CompetitionRatingCriterion;

use Zend\Db\Adapter\Adapter;
use Zend\Db\Adapter\ParameterContainer;
use Zend\Db\ResultSet\ResultSet;
use Zend\Db\Adapter\Driver\ResultInterface;
use Zend\Db\Exception;

class CompetitionRatingCriterionStorageMapper
{
	/** @var Adapter $db */
	protected $db;

	/**
	 * @param Adapter $db
	 */
	public function __construct($db)
	{
		$this->db = $db;
	}

	/**
	 * @param string $sql
	 * @param array $params
	 * @return array
	 * @throws \Exception
	 */
	protected function sqlSelect($sql, $params)
	{
		$stmt   = $this->db->createStatement($sql, new ParameterContainer($params));
		$result = $stmt->execute();
		if ($result instanceof ResultInterface && $result->isQueryResult()) {
			$resultSet = new ResultSet;
			$resultSet->initialize($result);
			$result = array();
			while ($ret = $resultSet->current()) {
				$result[] = (array)$ret;
				$resultSet->next();
			}
			return $result;
		} else {
			throw new \Exception("Internal error");
		}
	}

	/**
	 * @param int $competition_group_id
	 * @param int $id
	 * @throws \Exception
	 * @return CompetitionRatingCriterionEntity
	 */
	public function getItem($competition_group_id, $id)
	{
		$data = $this->sqlSelect('SELECT * FROM competition_rating_criteria WHERE id = ? AND competition_group_id = ?',
			array($id, $competition_group_id)
		);
		if (count($data) != 1) throw new \Exception("Not found");
		return CompetitionRatingCriterionEntity::fromDBArray($data[0]);
	}

	/**
	 * @param int $competition_group_id
	 * @param int $offset
	 * @param int $limit
	 * @return CompetitionRatingCriterionEntity[]
	 */
	public function getItems($competition_group_id, $offset, $limit) {
		$ret = $this->sqlSelect('SELECT * FROM competition_rating_criteria WHERE competition_group_id = ? ORDER BY `order` LIMIT ?, ?',
			array($competition_group_id, $offset, $limit)
		);
		$parts = array();
		foreach ($ret as $part) $parts[] = CompetitionRatingCriterionEntity::fromDBArray($part);
		return $parts;
	}

	/**
	 * @param CompetitionRatingCriterionEntity $item
	 * @return CompetitionRatingCriterionEntity
	 */
	public function updateItem($item)
	{
		$stmt = $this->db->createStatement('UPDATE competition_rating_criteria SET name = ?, order = ?, max_rating = ?,
			weight = ? WHERE id = ?, competition_group_id = ?', new ParameterContainer(array(
			$item->name, $item->order, $item->max_rating, $item->weight, $item->id, $item->competition_group_id
		)));
		$stmt->execute();
		return $this->getItem($item->competition_group_id, $item->id);
	}

	/**
	 * @param CompetitionRatingCriterionEntity $item
	 * @return CompetitionRatingCriterionEntity
	 */
	public function insertItem($item)
	{
		$stmt   = $this->db->createStatement('INSERT INTO competition_rating_criteria
			(competition_group_id, name, order, max_rating, weight, group_id) VALUES (?, ?, ?, ?, ?, ?)', new ParameterContainer(array(
			$item->competition_group_id, $item->name, $item->order, $item->max_rating, $item->weight, $item->group_id
		)));
		$result = $stmt->execute();

		$new_id = $result->getGeneratedValue();

		return $this->getItem($item->competition_group_id, $new_id);
	}

	/**
	 */
	public function getCount($competition_group_id)
	{
		$ret = $this->sqlSelect('SELECT COUNT(*) num FROM competition_rating_criteria WHERE competition_group_id = ?',
			array($competition_group_id)
		);
		return $ret[0]["num"];
	}


}
